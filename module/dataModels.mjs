...
export class ActorData extends foundry.abstract.TypeDataModel {
    ...
    return {
      ...
      resources: new SchemaField({
        ...
        hp: new SchemaField({
          ...
          percent: new NumberField({
            ...
            integer: false,
            initial: 1,
          }), // current hp as percent of max hp, decimal percent
        }),
        ...
        phys_armor: new SchemaField({
          ...
          temp: new NumberField({
            ...
            min: 0,
            initial: 0,
          }), // temporary armor (from skills)
          bonus: new NumberField({
            required: true,
            integer: true,
            min: 0,
            initial: 0,
          }), // misc. bonus max armor
          offset: new NumberField({
            required: true,
            integer: true,
            initial: 0,
          }), // offset from max phys_armor
        }),
      ...
      attributes: new SchemaField({
        bonuses: new SchemaField({
          crit_chance: new SchemaField({
            // chance to land a crit, expressed as a percentage
            value: new NumberField({
              required: true,
              integer: false,
              min: 0,
          ...
          crit_bonus: new SchemaField({
            // damage increase (on top of base) on crit (as percent)
            value: new NumberField({
              required: true,
              integer: false,
              min: 0,
          ...
          accuracy: new SchemaField({
            // base chance to hit (can go above 1)
            value: new NumberField({
              required: true,
              integer: false,
              min: -500,
          ...
          evasion: new SchemaField({
            // chance to dodge an attack (expressed as a percent)
            value: new NumberField({
              required: true,
              integer: false,
              min: -500,
          ...
          lifesteal: new SchemaField({
            // heal from damage done
            value: new NumberField({
              required: true,
              integer: false,
              min: -500,
        ...
        resistance: new SchemaField(
          Object.keys(
            (({ none, ...o }) => o)(CONFIG.CELESTUS.damageTypes),
          ).reduce((obj, type) => {
        ...
        statusImmune: new SchemaField(
          CONFIG.statusEffects.reduce((obj, status) => {
            obj[status.id] = new BooleanField({
              required: true,
              initial: false,
            });
            return obj;
        ...
        languages: new SchemaField(
          Object.keys(CONFIG.CELESTUS.languages).reduce((obj, language) => {
            obj[language] = new BooleanField({
              required: true,
              initial: language === "Common",
            });
            return obj;
          }, {}),
        ),
        ...
        movement: new SchemaField({
          // movement in map units (default ft)
          base: new NumberField({
            required: true,
            integer: false,
            min: 0,
        ...
        unspentPoints: new NumberField({
          required: true,
          integer: true,
          initial: 0,
        }), // derived unspent attribute points
        unspentCombat: new NumberField({
          required: true,
          integer: true,
          initial: 0,
        }),
        unspentCivil: new NumberField({
          required: true,
          integer: true,
          initial: 0,
        }),
      ...
      combat: new SchemaField(
        Object.keys(
          (({ none, ...o }) => o)(CONFIG.CELESTUS.combatSkills),
        ).reduce((obj, ability) => {
          ...
          obj[ability] = new SchemaField({
            ...
            base: new NumberField({
              ...
              integer: true,
              min: 0,
              max: CONFIG.CELESTUS.abilityMax,
              initial: 0,
            }), // base value from leveling
            bonus: new NumberField({
              required: true,
              integer: true,
              min: 0,
              initial: 0,
            }), // bonus to base value from items/features
            mod: new NumberField({
              required: true,
              integer: false,
              min: 0,
      ...
      civil: new SchemaField(
        Object.keys(
          (({ none, ...o }) => o)(CONFIG.CELESTUS.civilSkills),
        ).reduce((obj, ability) => {
          if (CONFIG.CELESTUS.combatSkills[ability]?.ignore) return obj;
          obj[ability] = new SchemaField({
            value: new NumberField({
      ...
      abilities: new SchemaField(
        Object.keys((({ none, ...o }) => o)(CONFIG.CELESTUS.abilities)).reduce(
          (obj, ability) => {
            obj[ability] = new SchemaField({
              value: new NumberField({
                required: true,
                integer: true,
                min: 0,
                max: CONFIG.CELESTUS.attributeMax,
                initial: 10,
              }), // base value
              mod: new NumberField({
                required: true,
  ...
  prepareDerivedData() {
    ...
    for (const item of this.parent.items) {
      ...
      if (item.type !== "skill" && item.system.equipped) {
        ...
        this.attributes.bonuses.crit_chance.value +=
          item.system.bonuses.crit_chance;
        this.attributes.bonuses.crit_bonus.value +=
          item.system.bonuses.crit_bonus;
        this.attributes.bonuses.accuracy.value += item.system.bonuses.accuracy;
        this.attributes.bonuses.initiative.value +=
          item.system.bonuses.initiative;
    ...
    if (this.parent.getFlag("celestus", "inspired")) {
      for (let ability of CONFIG.CELESTUS.inspiredAttributes) {
        this.abilities[ability].total +=
          1 + parseInt(this.attributes.level * CONFIG.CELESTUS.inspiredScalar);
        this.abilities[ability].bonus +=
          1 + parseInt(this.attributes.level * CONFIG.CELESTUS.inspiredScalar);
      }
    }
    ...
    for (let [key, ability] of Object.entries(this.abilities)) {
      ...
      if (this.parent.getFlag("celestus", "lone-wolf")) {
        ...
        spent = Math.min(
          spent * 2,
          CONFIG.CELESTUS.attributeMax - CONFIG.CELESTUS.baseAttributeScore,
        );
        ability.total +=
          spent + CONFIG.CELESTUS.baseAttributeScore - ability.value;
      }
    ...
    // calculate memory
    this.attributes.memory.total += parseInt(
      Math.floor(this.attributes.level / 2) + this.abilities.mind.total - 7,
    );
    // calculate modifiers
    // ability scores
    for (let [key, ability] of Object.entries(this.abilities)) {
      ability.mod +=
        (ability.total - CONFIG.CELESTUS.baseAttributeScore) *
          CONFIG.CELESTUS.abilityMod[key] +
        CONFIG.CELESTUS.baseAbilityMod[key];
    }
    // combat abilities
    for (let [key, ability] of Object.entries(this.combat)) {
      const mult =
        CONFIG.CELESTUS.combatSkills[key]?.modOverride ??
        CONFIG.CELESTUS.combatSkillMod;
      ability.mod += ability.value * mult;
    }
    // calculate lifesteal
    this.attributes.bonuses.lifesteal.value +=
      this.combat.deathbringer.mod + this.attributes.bonuses.lifesteal.bonus;
    // calculate crit chance
    this.attributes.bonuses.crit_chance.value +=
      this.abilities.wit.mod + this.attributes.bonuses.crit_chance.bonus;
    this.attributes.bonuses.crit_chance.value +=
      this.weaponType === "ranged" ? this.combat.ranged.mod : 0;
    // calculate crit bonus
    this.attributes.bonuses.crit_bonus.value +=
      1 +
      CONFIG.CELESTUS.baseCritBonus +
      this.attributes.bonuses.crit_bonus.bonus +
      this.combat.shroudstalker.mod;
    this.attributes.bonuses.crit_bonus.value +=
      this.weaponType === "twohand" ? this.combat.twohand.mod : 0;
    // calculate accuracy
    this.attributes.bonuses.accuracy.value +=
      CONFIG.CELESTUS.baseAccuracy + this.attributes.bonuses.accuracy.bonus;
    this.attributes.bonuses.accuracy.value +=
      this.weaponType === "onehand" ? this.combat.onehand.mod : 0;
    // calculate evasion
    this.attributes.bonuses.evasion.value +=
      this.attributes.bonuses.evasion.bonus;
    this.attributes.bonuses.evasion.value +=
      this.weaponType === "dualwield" ? this.combat.dualwield.mod : 0;
    // calculate overall damage bonus
    this.attributes.bonuses.damage.value +=
      this.attributes.bonuses.damage.bonus;
    // calculate movespeed
    ...
    if (this.parent.getFlag("celestus", "natural-armor")) {
      this.resources.phys_armor.max *=
        1 + this.abilities.con.mod * CONFIG.CELESTUS.naturalArmorScale;
      this.resources.mag_armor.max *=
        1 + this.abilities.con.mod * CONFIG.CELESTUS.naturalArmorScale;
    }
    ...
    this.attributes.movement.value *=
      (1 + this.attributes.movement.bonus) *
      (1 + this.combat.shroudstalker.mod);
    ...
    this.resources.phys_armor.value =
      this.resources.phys_armor.flat + this.resources.phys_armor.temp;
    this.resources.mag_armor.value =
      this.resources.mag_armor.flat + this.resources.mag_armor.temp;
  }
  ...
  get armor() {
    return {
      helmet: this.parent.items.filter(
        (i) => i.type === "armor" && i.system.slot === "helmet",
      ),
      chest: this.parent.items.filter(
        (i) => i.type === "armor" && i.system.slot === "chest",
      ),
      gloves: this.parent.items.filter(
        (i) => i.type === "armor" && i.system.slot === "gloves",
      ),
      leggings: this.parent.items.filter(
        (i) => i.type === "armor" && i.system.slot === "leggings",
      ),
      boots: this.parent.items.filter(
        (i) => i.type === "armor" && i.system.slot === "boots",
      ),
      amulet: this.parent.items.filter(
        (i) => i.type === "armor" && i.system.slot === "amulet",
      ),
      ring: this.parent.items.filter(
        (i) => i.type === "armor" && i.system.slot === "ring",
      ),
      belt: this.parent.items.filter(
        (i) => i.type === "armor" && i.system.slot === "belt",
      ),
      other: this.parent.items.filter(
        (i) => i.type === "armor" && i.system.slot === "none",
      ),
    };
  }
  ...
  get effects() {
    return {
      temporary: this.parent.effects.filter(
        (e) => !e.disabled && e.isTemporary,
      ),
  ...
  get features() {
    ...
    for (let [type, label] of Object.entries(CONFIG.CELESTUS.featureTypes)) {
      features[type] = this.parent.items.filter(
        (i) => i.type === "feature" && i.system.type === type,
      );
    }
    return features;
  }
  ...
  get combatAbilities() {
    return {
      weapon: Object.entries(this.combat).filter(
        (s) => CONFIG.CELESTUS.combatSkills[s[0]]?.type === "weapon",
      ),
      skill: Object.entries(this.combat).filter(
        (s) => CONFIG.CELESTUS.combatSkills[s[0]]?.type === "skill",
      ),
    };
  }
  ...
  get reach() {
    ...
    if (weapon?.system.reach)
      return this.attributes.reach.base + this.attributes.reach.bonus + 5;
  ...
  get bonusWeaponDamage() {
    ...
    // one weapon
      ...
      equipped.left.system.twoHanded ||
      !equipped.right ||
      equipped.right.type === "offhand"
    ) {
...
export class PlayerData extends ActorData {
  ...
  prepareDerivedData() {
    ...
    this.attributes.unspentPoints +=
      this.attributes.level * 2 +
      CONFIG.CELESTUS.baseAbilityPoints -
      spentPoints;
    // calculage ability bonus from enlightened
    if (this.parent.getFlag("celestus", "enlightened")) {
      for (let [ability, value] of Object.entries(
        CONFIG.CELESTUS.enlightenedBonus[this.attributes.level],
      )) {
        this.abilities[ability].bonus += value;
        this.abilities[ability].total += value;
      }
    ...
    for (let [key, ability] of Object.entries(this.combat)) {
      ...
      if (
        this.parent.getFlag("celestus", "lone-wolf") &&
        key !== "formshifter"
      ) {
        base *= 2;
    ...
    this.attributes.unspentCombat =
      this.attributes.level + CONFIG.CELESTUS.baseAbilityPoints - spentCombat;
    let spentCivil = 0;
    ...
    this.attributes.unspentCivil =
      Math.floor(this.attributes.level / 3) +
      CONFIG.CELESTUS.baseAbilityPoints -
      spentCivil;
   ...
   */
  get equipped() {
    const rings = this.parent.items.filter(
      (i) => i.type === "armor" && i.system.equipped && i.system.slot == "ring",
    );
    const equippedWeapons = this.parent.items.filter(
      (i) => i.type == "weapon" && i.system.equipped,
    );
    const offhand = this.parent.items.find(
      (i) => i.type === "offhand" && i.system.equipped,
    );
    let leftHand;
    let rightHand;
    ...
    return {
      helmet: this.parent.items.find(
        (i) =>
          i.type === "armor" && i.system.equipped && i.system.slot == "helmet",
      ),
      chest: this.parent.items.find(
        (i) =>
          i.type === "armor" && i.system.equipped && i.system.slot == "chest",
      ),
      gloves: this.parent.items.find(
        (i) =>
          i.type === "armor" && i.system.equipped && i.system.slot == "gloves",
      ),
      belt: this.parent.items.find(
        (i) =>
          i.type === "armor" && i.system.equipped && i.system.slot == "belt",
      ),
      leggings: this.parent.items.find(
        (i) =>
          i.type === "armor" &&
          i.system.equipped &&
          i.system.slot == "leggings",
      ),
      boots: this.parent.items.find(
        (i) =>
          i.type === "armor" && i.system.equipped && i.system.slot == "boots",
      ),
      amulet: this.parent.items.find(
        (i) =>
          i.type === "armor" && i.system.equipped && i.system.slot == "amulet",
      ),
      ring1: rings[0],
      ring2: rings[1],
      left: leftHand,
  ...
  get skills() {
    const skills = {
      memorized: this.parent.items.filter(
        (i) => (i.type === "skill", i.system.memorized === "true"),
      ),
      always: this.parent.items.filter(
        (i) => (i.type === "skill", i.system.memorized === "always"),
      ),
    };
    if (this.parent.getFlag("celestus", "eiditic")) {
      skills.eiditic = this.parent.items.filter(
        (i) => (
          i.type === "skill",
          i.system.memorized === "false",
          i.system.type === "civil"
        ),
      );
      skills.unmemorized = this.parent.items.filter(
        (i) => (
          i.type === "skill",
          i.system.memorized === "false" && i.system.type !== "civil"
        ),
      );
    } else {
      skills.unmemorized = this.parent.items.filter(
        (i) => (i.type === "skill", i.system.memorized === "false"),
      );
    }
    return skills;
  }
  ...
  get knownSkills() {
    ...
    return {
      count:
        skills.memorized.length +
        skills.unmemorized.length +
        (skills.eiditic ? skills.eiditic.length : 0),
...
export class NpcData extends ActorData {
  static defineSchema() {
    let schema = super.defineSchema();
    schema.abilitySpread = new SchemaField(
      Object.keys((({ none, ...o }) => o)(CONFIG.CELESTUS.abilities)).reduce(
        (obj, ability) => {
          obj[ability] = new NumberField({
            required: true,
            min: 0,
            initial: 0,
          });
  ...
  prepareDerivedData() {
    ...
    for (let [key, value] of Object.entries(this.abilitySpread)) {
      this.abilities[key].total += Math.round(
        CONFIG.CELESTUS.baseAttributeScore +
          value * CONFIG.CELESTUS.npcAttributeScalar * this.attributes.level,
      );
    }

    // calculate combat abilities
    for (let [key, ability] of Object.entries(this.combat)) {
      ability.value += Math.round(
        CONFIG.CELESTUS.npcAbilityBase +
          ability.base *
            CONFIG.CELESTUS.npcAbilityScalar *
    ...
    // calculate armor
    this.resources.phys_armor.max += Math.round(
      this.armorSpread.phys *
        CONFIG.CELESTUS.npcArmorScalar *
        CONFIG.CELESTUS.e ** this.attributes.level *
        this.armorBoost,
    );
    this.resources.mag_armor.max += Math.round(
      this.armorSpread.mag *
        CONFIG.CELESTUS.npcArmorScalar *
        CONFIG.CELESTUS.e ** this.attributes.level *
        this.armorBoost,
    );
...
export class SkillData extends foundry.abstract.TypeDataModel {
  static defineSchema() {
    return {
      ...
      prereqs: new SchemaField({
        // prerequisite combat skill values to memorize
        flamespeaker: new NumberField({
          required: true,
          integer: true,
          min: 0,
          initial: 0,
        }),
        tidecaller: new NumberField({
          required: true,
          integer: true,
          min: 0,
          initial: 0,
        }),
        stormseeker: new NumberField({
          required: true,
          integer: true,
          min: 0,
          initial: 0,
        }),
        duneshaper: new NumberField({
          required: true,
          integer: true,
          min: 0,
          initial: 0,
        }),
        voidcantor: new NumberField({
          required: true,
          integer: true,
          min: 0,
          initial: 0,
        }),
        deathbringer: new NumberField({
          required: true,
          integer: true,
          min: 0,
          initial: 0,
        }),
        shroudstalker: new NumberField({
          required: true,
          integer: true,
          min: 0,
          initial: 0,
        }),
        lightbringer: new NumberField({
          required: true,
          integer: true,
          min: 0,
          initial: 0,
        }),
        formshifter: new NumberField({
          required: true,
          integer: true,
          min: 0,
          initial: 0,
        }),
        huntmaster: new NumberField({
          required: true,
          integer: true,
          min: 0,
          initial: 0,
        }),
        warlord: new NumberField({
          required: true,
          integer: true,
          min: 0,
          initial: 0,
        }),
        scoundrel: new NumberField({
          required: true,
          integer: true,
          min: 0,
          initial: 0,
        }),
        lore: new NumberField({
          required: true,
          integer: true,
          min: 0,
          initial: 0,
        }),
        nature: new NumberField({
          required: true,
          integer: true,
          min: 0,
          initial: 0,
      ...
      targets: new SchemaField({
        // amount of targets required/allowed
      ...
      overridesWeaponDamage: new BooleanField({
        required: true,
        initial: false,
      }),
      overridesWeaponType: new BooleanField({ required: true, initial: false }),
      overridesWeaponRange: new BooleanField({
        required: true,
        initial: false,
      }),
      ...
      linger: new SchemaField({
        ...
        lingerDuration: new NumberField({
          required: true,
          integer: true,
          initial: 0,
        }), // 0 = store as non-temp
        surfaceType: new StringField({ required: true, initial: "none" }),
        children: new ArrayField(new StringField()), // array of ids of owned effects created by the aura
        texture: new FilePathField({ categories: ["IMAGE"] }),
      }),
      channelDuration: new NumberField({
        required: true,
        initial: 0,
        integer: true,
        min: 0,
  ...
  get finalRange() {
    ...
    if (
      this.type === "weapon" &&
      !this.overridesWeaponRange &&
      this.parent.actor?.system?.equipped?.left
    ) {
      ...
      if (
        this.type === "magic" &&
        this.parent.actor &&
        this.parent.actor.getFlag("celestus", "farsight")
      ) {
    ...
    if (this.targets.type === "self") {
      ...
      this.targets.type === "none" ||
      this.targets.type === "radius" ||
      this.targets.type === "line"
    ) {
      return false;
    } else {
      if (range > 0) {
  ...
  get finalAP() {
    ...
    if (actor.getFlag("celestus", "elementalist")) {
      ...
      if (
        CONFIG.CELESTUS.surfaceTypes[standingOn]?.schools?.includes(
          this.school,
        ) &&
        ap > 1
      ) {
  ...
  get disabled() {
    ...
    if (
      ...
      actor.type === "player" &&
      !(this.type === "civil" && actor.getFlag("celestus", "eiditic"))
    ) {
  ...
  get combatPrereqs() {
    return Object.entries(this.prereqs).filter(
      ([k, v]) => CONFIG.CELESTUS.combatSkills[k],
    );
  }
  get civilPrereqs() {
    return Object.entries(this.prereqs).filter(
      ([k, v]) => CONFIG.CELESTUS.civilSkills[k],
    );
  }
  ...
  get totalDamage() {
    ...
    if (this.needsDamageField) {
      ...
      for (let part of this.damage) {
        ...
        const mult = actor
          ? calcMult(actor, part.type, this.ability, part.value, false, 0)
          : 1;
      ...
      if (!actor) return;
      return calcWeaponDamage(
        this.parent.actor,
        this.weaponEfficiency,
        this.overridesWeaponType ? this.overrideDamageType : false,
      );
    }
  }
}
  ...
  static defineSchema() {
    return foundry.utils.mergeObject(super.defineSchema(), {
      ...
      bonuses: new SchemaField({
        ...
        statusImmune: new ArrayField(new StringField()),
      ...
      ownedEffects: new ArrayField(new StringField()),
      // for item generation
      socketTypes: new ArrayField(new StringField(), {
        required: true,
        initial: ["", "", "", "", "", "", "", ""],
      }),
      socketValues: new ArrayField(new StringField(), {
        required: true,
        initial: ["", "", "", "", "", "", "", ""],
      }),
      plugIds: new ArrayField(new StringField(), {
        required: true,
        initial: ["", "", "", "", "", "", "", ""],
      }),
      plugChanges: new ArrayField(new ObjectField(), {
        required: true,
        initial: [{}, {}, {}, {}, {}, {}, {}, {}],
      }),
      level: new NumberField({
      ...
      grantedSkills: new ArrayField(
        new SchemaField({
          // skills granted by this effect
          uuid: new StringField(),
          name: new StringField(),
        }),
        { required: true, initial: [] },
      ),
      ownedItems: new ArrayField(new StringField(), { // items that have been created by this effect
        required: true,
        initial: [],
      }),
      // # of runes
      runeSlots: new NumberField({
        required: true,
        initial: 0,
        integer: true,
        min: 0,
      ...
      slottedRunes: new ArrayField(new StringField(), {
        required: true,
        initial: [],
      }),
    });
  ...
  async _preCreate(data, options, user) {
    ...
    if (grantedEffects && data.system?.equipped) {
      ...
      for (const effect of grantedEffects) {
        ...
        const [newEffect] = await this.parent.parent?.createEmbeddedDocuments(
          "ActiveEffect",
          [effect],
        );
        if (newEffect) {
    ...
    if (grantedSkills && data.system?.equipped === true) {
      ...
      for (const item of grantedSkills) {
        ...
        const newItems = await this.parent.parent?.createEmbeddedDocuments(
          "Item",
          [sourceItem.toJSON()],
        );
        if (newItems) {
...
export class ArmorData extends GearData {
  ...
  get value() {
    ...
    return {
      phys: Math.round(
        ((this.base.phys * scalar) / 100.0) *
          CONFIG.CELESTUS.e ** level *
          this.efficiency,
      ),
      mag: Math.round(
        ((this.base.mag * scalar) / 100.0) *
          CONFIG.CELESTUS.e ** level *
          this.efficiency,
      ),
    };
...
export class WeaponData extends GearData {
  static defineSchema() {
    ...
    schema.bonusElements = new ArrayField(
      new SchemaField({
        type: new StringField({ required: true, initial: "none" }), // damage type
  ...
  get damage() {
    // get actor level if it exists
    const level = this.parent.actor
      ? this.parent.actor.system.attributes.level
      : 1;
    const dice =
      1 +
      Math.round(
        CONFIG.CELESTUS.weaponDmgScalar * Math.pow(CONFIG.CELESTUS.e, level),
      );
    ...
    let abilityBonus = weaponType
      ? this.parent?.actor?.system?.combat[weaponType]?.value *
        CONFIG.CELESTUS.combatSkillMod
      : 0;
    let flatBonus =
      abilityBonus +
      (this.parent?.actor?.system.attributes.bonuses.damage.value ?? 0);
    flatBonus += (this.parent?.actor?.system.dmgBoost ?? 0) * 0.5;
    // calculate mult
    let mult = this.parent.actor
      ? calcMult(
          this.parent.actor,
          this.type,
          this.ability,
      ...
      : 1;
    let multCrit = this.parent.actor
      ? calcMult(
          this.parent.actor,
          this.type,
          this.ability,
  ...
  get statusApplyRolls() {
    ...
    for (const status of this.appliedStatuses) {
      ret.push({
        id: status,
        chance:
          CONFIG.CELESTUS.statusChance[this.twoHanded ? "twoHand" : "oneHand"],
      });
    }
    return ret;
  }
  ...
  get bonusDamage() {
    ...
    for (const element of this.bonusElements) {
      ...
      damage.push({
        ...
        max: Math.max(
          Math.floor((flat + CONFIG.CELESTUS.weaponBonusDmgDie) * mult) - 1,
          1,
        ),
        avg: Math.max(
          Math.floor(
            (flat + (CONFIG.CELESTUS.weaponBonusDmgDie / 2 + 0.5)) * mult,
          ) - 1,
          1,
        ),
...
export class EffectData extends foundry.abstract.TypeDataModel {
  static defineSchema() {
    return {
      ...
      removes: new ArrayField(new StringField()), // statuses it removes
      blocks: new ArrayField(new StringField()), // statuses it prevents from ebing applied
      triggers: new ArrayField(new StringField()), // statuses it brings with it
  ...
  async _preCreate(data, options, user) {
    ...
    if (targetsSelf || data.origin !== actor.uuid || !data.system?.aura?.has) {
      switch (data.system.resistedBy) {
        ...
        case "any":
          if (
            actor.system.resources.phys_armor.value > 0 ||
            actor.system.resources.mag_armor.value > 0
          )
            resisted = true;
      }
    }
        ...
        if (tormentor) {
          ...
          if (data.duration) {
            this.parent.updateSource({
              "duration.rounds": data.duration.rounds + 1,
            });
          }
        }
      }
    ...
    if (data.statuses && data.statuses.length > 0) {
      for (const status of data.statuses) {
        ...
        if (actor.system.attributes.statusImmune[status] === true) {
          canvasPopupText(
            actor,
            `Immune to ${CONFIG.statusEffects.find((s) => s.id === status).name}`,
          );
          return false;
        }
        ...
        for (let effect of actor.effects) {
          if (effect.system.blocks.find((b) => b === status)) {
            canvasPopupText(
              actor,
              `${data.name} blocked by ${effect.name}`,
              "#fff",
              broadcast,
            );
            return false;
    ...
    if (data.system?.combines) {
      for (let combination of data.system.combines) {
        ...
        let combiner = actor.effects.find((e) =>
          e.statuses.has(combination.with),
        );
        // second incredient exists, combine
        if (combiner) {
          combiner.delete();
          const product = await actor.toggleStatusEffect(combination.makes, {
            active: true,
          });
    ...
    if (this.parent.parent?.documentName === "Actor") {
      ...
      if (active && active.actorId === this.parent.parent.id) {
        ...
        for (let part of effect.system.damage) {
          ...
          const level = origin
            ? origin.system.attributes.level
            : this.parent.parent.system.attributes.level;
          // damage type
          const type = part.type;
...
export class RuneData extends CelestusItemData {
  static defineSchema() {
    return foundry.utils.mergeObject(super.defineSchema(), {
      ...
      plugs: new SchemaField({
        weapon: new ArrayField(
          new StringField({ required: true, initial: "none" }),
        ),
        armor: new ArrayField(
          new StringField({ required: true, initial: "none" }),
        ),
        amulet: new ArrayField(
          new StringField({ required: true, initial: "none" }),
        ),
        offhand: new ArrayField(
          new StringField({ required: true, initial: "none" }),
        ),
      }),
  ...
  get slotted() {
    ...
    return this.parent.parent.items.find((i) =>
      i.system?.slottedRunes?.includes(this.parent.id),
    );
  }
}
