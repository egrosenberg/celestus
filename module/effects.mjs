...
export class CelestusEffect extends ActiveEffect {
  ...
  async _preUpdate(changed, options, user) {
    ...
    if (
      typeof changed.disabled !== "undefined" &&
      changed.disabled !== this.disabled
    ) {
      if (changed.disabled === false) {
        // enabling effect, grant all items
  ...
  async _preCreate(data, options, user) {
    ...
    if (grantedSkills && !data.disabled) {
      ...
      for (const item of grantedSkills) {
        const sourceItem = await fromUuid(item.uuid);
        const newItems = await this.parent?.createEmbeddedDocuments("Item", [
          sourceItem.toJSON(),
        ]);
        if (newItems) {
          // mark new Item as always prepped
