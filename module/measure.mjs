...
export class CelestusMeasuredTemplate extends foundry.canvas.placeables
  .MeasuredTemplate {
  /**
  ...
  static _constructPreset(item) {
    ...
    if (item.system.targets.type === "radius") {
      size +=
        item.actor?.getActiveTokens()[0]?.w /
    ...
    const templateData = {
      user: game.user?.id,
      width:
        item.system.targets.type === "cube"
          ? item.system.targets.size
          : item.system.targets.width,
      direction: 0,
      x: 0,
      y: 0,
      distance: size,
      ...
      t:
        CONFIG.CELESTUS.targetTypes[item.system.targets.type]?.measure ??
        "circle",
    ...
    if (CONFIG.CELESTUS.targetTypes[item.system.targets.type]?.angle) {
      templateData.angle =
        CONFIG.CELESTUS.targetTypes[item.system.targets.type]?.angle;
    }
  ...
  testTemplate(testTemplate) {
    ...
    const AABB = x1 < testX2 && x2 > testX1 && y2 > testY1 && y1 < testY2;
    ...
    // two rectangles
      ...
      if (
        shape.contains(testX1, testY1) &&
        shape.contains(testX1, testY2) &&
        shape.contains(testX2, testY2) &&
        shape.contains(testX2, testY1)
      ) {
        return 1;
      }
      // check if testShape contains the entirety of testShape
      if (
        testShape.contains(x1, y1) &&
        testShape.contains(x1, y2) &&
        testShape.contains(x2, y2) &&
        testShape.contains(x2, y1)
      ) {
        return -1;
      }
      // they just intersect
  ...
  async combineSurface(template, canDelete = false) {
    ...
    if (!thisType || !testType || thisType === "none" || testType === "none")
      return;
    ...
    if (
      !surface.combines?.[testType] &&
      !testSurface.combines?.[thisType] &&
      thisType !== testType
    )
      return;
  ...
  async spreadEffectsTo(token, newPos = undefined, oldPos = undefined) {
    ...
    if (
      !this.document.getFlag("celestus", "hasEffects") &&
      !this.document.getFlag("celestus", "surfaceType")
    )
      return false;
    ...
    if (surfaceType) {
      if (standingOn) {
        ...
        const thisStartTurn =
          this.document.getFlag("celestus", "lingerStartTurn") ?? 0;
        ...
        if (cSurface) {
          ...
          setThis ||=
            thisStartRound === testStartRound && thisStartTurn > testStartTurn;
        }
        if (setThis) {
        ...
        token.actor.getFlag("celestus", "surfaceId") === this.document.uuid
      ) {
    ...
    if (
      ...
      if (skill) {
        if (skill.system.linger.targets === "any") {
          ...
          skill.system.linger.targets === "ally" &&
          token.disposition === actor.prototypeToken.disposition
        ) {
          validTarget = true;
        } else if (
          skill.system.linger.targets === "enemy" &&
          token.disposition !== actor.prototypeToken.disposition
        ) {
          validTarget = true;
        } else if (
          skill.system.linger.targets === "type" &&
          skill.system.linger.targetType === actor.system.t
        ) {
          validTarget = true;
        }
      } else {
        ...
        if (old.length > 0) {
          for (const e of old) {
            ...
            if (skill && skill.system.linger.lingerDuration !== 0) {
              await e.update({
                "duration.rounds": skill.system.linger.lingerDuration,
              });
            }
          }
        } else {
          ...
          if (skill) {
            ...
            for (const effect of skill.effects) {
              if (
                effect.disabled ||
                token.actor.effects.find((i) => i.name === effect.name)
              ) {
                continue;
              }
          ...
          if (skill.system.linger.lingerDuration > 0) {
            for (const e of lingering) {
              if (!lingering.duration.rounds) {
                ...
                lingering.duration.rounds > skill.system.linger.lingerDuration
              ) {
                await lingering.update({
                  "duration.rounds": skill.system.linger.lingerDuration,
                });
              }
            }
...
export class CelestusMeasuredTemplateDocument extends MeasuredTemplateDocument {
  ...
  async _preDelete(options, user) {
    ...
    if (skill) {
      ...
      CONFIG.CELESTUS.surfaceTypes[this.getFlag("celestus", "surfaceType")]
        ?.statuses
    ) {
      cleanup = false;
    }
