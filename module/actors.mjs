...
export class CelestusActor extends Actor {
  ...
  async _preUpdate(changed, options, user) {
      ...
      if (statSpread && this.system.spread != statSpread) {
        if (CONFIG.CELESTUS.npcStats[statSpread]) {
          ...
          for (const key of Object.keys(CONFIG.CELESTUS.damageTypes)) {
            resists[key] = {
              base: findSpreadStat(statSpread, `${key}Resistance`) || 0,
            };
          }
          let combatAbilities = {};
          for (const key of Object.keys(CONFIG.CELESTUS.combatSkills)) {
            combatAbilities[key] = {
              base: findSpreadStat(statSpread, key) || 0,
            };
          }
          ...
          if (!changed.system.attributes.bonuses)
            changed.system.attributes.bonuses = {};
          if (!changed.system.attributes.bonuses.initiative)
            changed.system.attributes.bonuses.initiative = {};
          changed.system.attributes.bonuses.initiative.bonus =
            findSpreadStat(statSpread, "initiative") || 1;

          if (!changed.system.resources) changed.system.resources = {};
          if (!changed.system.resources.ap) changed.system.resources.ap = {};
          changed.system.resources.ap.max =
            findSpreadStat(statSpread, "apMax") || 6;
          changed.system.resources.ap.start =
            findSpreadStat(statSpread, "apStart") || 4;
        }
      }
    }
    ...
    if (this.type === "player") {
      ...
      if (level) {
        const maxFP =
          CONFIG.CELESTUS.maxFP.find((i) => i[0].includes(level))?.[1] ?? 0;
        resources ??= {};
        resources.fp ??= {};
  ...
  async applyDamage(damage, type, origin, options = {}) {
    ...
    if (CONFIG.CELESTUS.damageTypes[type].style === "physical") {
      ...
      if (tPhysArmor > 0) {
        ...
        await this.update({
          "system.resources.phys_armor.temp": parseInt(newTPhys),
        });
      }
      ...
      if (remaining > 0) {
        ...
        // update phys armor
        await this.update({
          "system.resources.phys_armor.flat": parseInt(newPhysArmor),
        });
      }
    }
    // magical damage
      ...
      if (tMagArmor > 0) {
        ...
        // update temp phys armor
        await this.update({
          "system.resources.mag_armor.temp": parseInt(newTMag),
        });
      }
      // account for damage past temp phys armor
      if (remaining > 0) {
        ...
        // update mag armor
        await this.update({
          "system.resources.mag_armor.flat": parseInt(newMagArmor),
        });
      }
    }
    // healing
      ...
      if (CONFIG.CELESTUS.damageTypes[type].text === "phys_armor") {
        ...
        if (remaining > 0) {
          ...
          newPhysArmor =
            newPhysArmor > maxPhysArmor ? maxPhysArmor : newPhysArmor;
        ...
        // damage directly to physical armor
          ...
          await this.update({
            "system.resources.phys_armor.temp": parseInt(newTPhysArmor),
          });
          // zero out remaining
          remaining = 0;
        }
      ...
      // magic armor restoration (cant go above max physical armor)
        ...
        // damage directly to magic armor
          ...
          await this.update({
            "system.resources.mag_armor.temp": parseInt(newTMagArmor),
          });
          // zero out remaining
          remaining = 0;
        }
    ...
    if (origin) {
      ...
      if (
        value > 0 &&
        this.system.resources.hp.flat < 1 &&
        origin.getFlag("celestus", "executioner") &&
        !origin.getFlag("celestus", "executedThisTurn") &&
      ...
      if (
        ...
        const heal =
          (origin.system.attributes.bonuses.lifesteal.value + lifesteal) *
          damage;
        await origin.update({
          "system.resources.hp.flat":
            origin.system.resources.hp.flat + Math.round(heal),
        });
        if (!options.reflected && damage > 0) {
          const retribution = this.system.combat.retributive.mod * damage;
          await origin.applyDamage(retribution, type, this, {
            reflected: true,
          });
        }
      }
  ...
  async useSkill(skill, options = {}) {
    ...
    if (this.inCombat) {
      ...
      if (useResources) {
        await this.update({
          "system.resources.ap.value":
            actor.resources.ap.value - skill.system.finalAP,
        });
      }
    }
    // use fp
    if (useResources) {
      await this.update({
        "system.resources.fp.value":
          actor.resources.fp.value - skill.system.finalFP,
      });
    }
    ...
    if (useResources) {
      ...
      if (skill.system.type === "civil") {
        skill.update({
          "system.cooldown.value": skill.system.cooldown.max !== 0 ? -1 : 0,
        });
      }
    ...
    if (item.type === "armor") {
      if (item.system.slot === "ring") {
        if (equipped[`${item.system.slot}${n}`]) {
          equipped[`${item.system.slot}${n}`].update({
  ...
  async setWeaponSkill() {
    ...
    if (right) {
      if (right.type === "offhand") {
        ...
        if (right.system.twoHanded) {
          if (
            right.system.ability === "str" ||
            right.system.ability === "dex"
          ) {
            wepSkill = "twohand";
    ...
    if (wepSkill) {
      ...
      if (cSkill) {
        ...
        const [newSkill] = await this.createEmbeddedDocuments("Item", [
          cSkill.toJSON(),
        ]);
        if (newSkill) {
  ...
  async refresh(dawn = false) {
    ...
    this.update({
      "system.resources.phys_armor.flat": this.system.resources.phys_armor.max,
    });
    // remove temp physical armor
    this.update({ "system.resources.phys_armor.temp": 0 });
    // update magic armor
    this.update({
      "system.resources.mag_armor.flat": this.system.resources.mag_armor.max,
    });
    ...
    // refresh ap
    this.update({
      "system.resources.ap.value": this.system.resources.ap.start,
    });
    ...
    for (let item of this.items) {
      if (
        item.type === "skill" &&
        (dawn || item.system.type !== "civil" || item.system.cooldown.max > 0)
      ) {
  ...
  async startTurn() {
    ...
    await this.update({
      "system.resources.ap.value": Math.min(ap + startAp, maxAp),
    });
  ...
  async rollInitiative() {
    ...
    for (const token of tokens) {
      try {
        ...
        let r = new Roll(game.system.initiative, this.system, {
          flavor: `${this.name} rolls initiative`,
        });
        await r.toMessage({
...
export class CelestusToken extends foundry.canvas.placeables.Token {
  ...
  drawPointer() {
    ...
    [this.pointerPixi.x, this.pointerPixi.y] = [
    ...
    for (const effect of effects) {
      ...
      for (const target of token.parent?.tokens) {
        ...
        if (targets === "any") {
          ...
          targets === "ally" &&
          token.disposition === target.disposition
        ) {
          validTarget = true;
        } else if (
          targets === "enemy" &&
          ...
          targets === "type" &&
          effect.system.aura.targetType === target.actor.system.t
        ) {
          validTarget = true;
        }
        // check distance
        const distance = canvas.grid
          .measurePath([tokenCoords, { x: target.x, y: target.y }])
          .distance.toFixed(1);
          ...
          if (old.length > 0) {
            for (const e of old) {
              ...
              if (effect.system.aura.lingerDuration !== 0) {
                await e.update({
                  "duration.rounds": effect.system.aura.lingerDuration,
                });
              }
            }
          } else {
    ...
    for (const origin of token.parent?.tokens) {
      ...
      for (const effect of effects) {
        ...
        if (targets === "any") {
          ...
          targets === "ally" &&
          token.disposition === origin.disposition
        ) {
          validTarget = true;
        } else if (
          targets === "enemy" &&
          ...
          targets === "type" &&
          effect.system.aura.targetType === origin.actor.system.t
        ) {
          validTarget = true;
        }
        // check distance
        const distance = canvas.grid
          .measurePath([tokenCoords, { x: origin.x, y: origin.y }])
          .distance.toFixed(1);
        if (distance > effect.system.aura.radius) validTarget = false;
