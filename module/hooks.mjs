...
export async function rollAttack(e) {
  ...
  const critThresh =
    100 - actor.system.attributes.bonuses.crit_chance.value * 100;
  ...
  if (item.type === "skill") {
    ...
    if (item.system.type === "magic" || item.system.overridesWeaponDamage) {
      ...
      for (let part of item.system.damage) {
        ...
        const base = CONFIG.CELESTUS.baseDamage.formula[
      ...
      let overrideDamageType = item.system.overridesWeaponType
        ? item.system.overrideDamageType
        : "";
...
export async function applyDamageComponent(e) {
  ...
  if (!parent)
    return console.warn("CELESTUS | Could not find damage component parent");
  ...
  // iterate through each controlled token
  for (const token of selected) {
    await token.actor.applyDamage(damage, type, origin, {
      lifesteal: lifesteal,
    });
  }
...
export async function applyStatusHook(e) {
  ...
  for (const target of selected) {
    ...
    for (const status of item.effects) {
      if (
        status.disabled ||
        target.actor.effects.find((i) => i.name === status.name)
      ) {
        continue;
      }
      await target.actor.createEmbeddedDocuments("ActiveEffect", [
...
export async function addChatButtons(msg, html, options) {
  ...
  if (msg.system.isAttack) {
    ...
    if (total < msg.getFlag("celestus", "hitThreshold")) {
      ...
      html
        .find(".dice-total")
        .html(total + ' <i class="fa-solid fa-xmark attack-indicator"></i>');
    } else if (total > msg.getFlag("celestus", "critThreshold")) {
      html.find(".dice-total").css("background-color", BLUE);
      html
        .find(".dice-total")
        .html(total + ' <i class="fa-solid fa-star attack-indicator"></i>');
    } else {
      html.find(".dice-total").css("background-color", GREEN);
      html
        .find(".dice-total")
        .html(total + ' <i class="fa-solid fa-check attack-indicator"></i>');
    }
  ...
  if (msg.system.isSkill) {
    ...
    const disabled =
      perms.default >= 3 || (game.user.id in perms && perms[game.user.id] >= 3)
        ? ""
        : "disabled";
  ...
  if (msg.system.isDamage) {
    ...
    let label =
      CONFIG.CELESTUS.damageTypes[msg.system.damageType]?.label ??
      msg.system.damageType;
  ...
  $("button.apply-damage").each(async function () {
    ...
    if (controlled) {
      ...
      for (const term of damageParts.terms) {
        ...
        part =
          CONFIG.CELESTUS.damageTypes[term.type].style === "healing"
            ? -part
            : part;
        damage += part;
      }
...
export async function populateHotbar(actor) {
  ...
  if (
    !game.settings.get("celestus", "autoPopulateHotbar") &&
    game.settings.get("celestus", "hotbarPopulateConfirmation")
  ) {
  ...
  actor =
    ...
    null;
  if (!actor)
    return ui.notifications.warn("CELESTUS | No actor to populate hotbar from");
...
export async function triggerTurn(combat, prior, current) {
  ...
  if (
    ...
    (prior.turn > current.turn && prior.round === current.round)
  )
    return;
  ...
  for (const template of canvas.scene.templates) {
    ...
    if (template.getFlag("celestus", "linger") === true) {
      ...
      if (
        current.round - startRound > duration ||
        (current.round - startRound >= duration && current.turn > startTurn)
      ) {
        // check if surface turns to something else on death
        const onEnd =
          CONFIG.CELESTUS.surfaceTypes[
            template.getFlag("celestus", "surfaceType")
          ]?.onEnd;
...
export async function renderResourcesUi(hide = false) {
  ...
  $("#ui-resources")
    ...
    .each((index, target) => {
      ...
      const resourceVal =
        actor.system.resources[varPath.substring(varPath.lastIndexOf(".") + 1)]
          .value;
      const resourceMax =
        actor.system.resources[varPath.substring(varPath.lastIndexOf(".") + 1)]
          .max;
...
export async function resourceInteractAp(ev) {
  ...
  if (index === actor.system.resources.ap.value) {
    await actor.update({
      "system.resources.ap.value": actor.system.resources.ap.value - 1,
    });
  } else {
...
export async function renderTokenInfo(token, hovered, force) {
  ...
  $("#ui-token-hover")
    ...
    .each((index, target) => {
      ...
      $(target)
        .parents(".resource")
        .css("background-image", `${curCSS}, ${gradient}`);
      if (resourcePercent < 0) {
...
export async function updateBossResources(actor) {
  ...
  for (const key of ["phys_armor", "mag_armor", "hp"]) {
    ...
    $(ui)
      .find(`#${key} .foreground`)
      .animate({ width: percent + "%" }, 750, "swing");
    await sleep(250);
    $(ui)
      .find(`#${key} .trailer`)
      .animate({ width: percent + "%" }, 1750, "swing");
  }
...
async function teleportTicker() {
  while (teleporting) {
    await sleep(20);
    CONFIG.CELESTUS.teleportCursor.rotation +=
      CONFIG.CELESTUS.teleportCursorRotationSpeed;
  }
}
