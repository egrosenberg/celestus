...
export class CelestusItem extends Item {
  ...
  prepareDerivedData() {
    ...
    if (["armor", "offhand", "weapon"].includes(this.type)) {
      const plugType = ["amulet", "ring"].includes(this.system.spread ?? "")
        ? "amulet"
        : this.type;
      ...
      for (const r_id of this.system.slottedRunes) {
        ...
        if (changes) {
          for (const change of changes) {
            ...
            // get current value
            const key = change.id.startsWith("system.")
              ? change.id.slice(7)
              : change.id;
  ...
  async _preUpdate(changed, options, user) {
    ...
    if (this.type === "armor") {
      ...
      if (spread && spread !== "none") {
        const base =
          CONFIG.CELESTUS.armor.spreads[spread] ??
          CONFIG.CELESTUS.armor.spreads.none;
        changed.system.base = {
          phys: base.phys,
          mag: base.mag,
        };
    ...
    if (this.type === "offhand") {
      ...
      if (spread && spread !== "none") {
        const base =
          CONFIG.CELESTUS.offhand.spreads[spread] ??
          CONFIG.CELESTUS.offhand.spreads.none;
        changed.system.base = {
          phys: base.phys,
          mag: base.mag,
        };
    ...
    if (changed.system?.equipped !== this.system.equipped) {
      if (
        ...
        for (const item of this.system.grantedSkills) {
          ...
          const newItem = await this.parent?.createEmbeddedDocuments("Item", [
            sourceItem.toJSON(),
          ]);
          if (newItem) {
      ...
      } else if (changed.system?.equipped === false) {
    ...
    if (["armor", "offhand", "weapon"].includes(this.type)) {
      ...
      if (
        (socketSpread && socketSpread !== this.system.socketSpread) ||
        newRarity
      ) {
        if (!socketSpread) {
    ...
    if (
      ["armor", "offhand", "weapon"].includes(this.type) &&
      this.parent?.documentName === "Actor"
    ) {
  ...
  _onCreate(data, options, userid) {
    ...
    this.updateSource({
      "system.ownedEffects": options.system?.ownedEffects ?? [],
    });
  }
          ...
          buttons: [
            {
              ...
              callback: (event, button, dialog) => [
                button.form.elements.useItem.checked,
                button.form.elements.useResources.checked,
              ],
            },
            {
              action: "cancel",
              ...
              if (error) {
                ...
                await this.parent.update({
                  "system.resources.ap.value":
                    this.parent.system.resources.ap.value - this.system.ap,
                });
              }
            }
            // create chat message
  ...
  async autoSelectSocket(index) {
    ...
    const validSockets = CONFIG.CELESTUS.itemSockets.filter(
      (s) =>
