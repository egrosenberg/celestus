...
export class CharacterSheet extends foundry.appv1.sheets.ActorSheet {
  ...
  activateListeners(html) {
    ...
    html.on("click", "#refresh-all", async () => {
      const proceed = await foundry.applications.api.DialogV2.confirm({
        ...
        content:
          "Are you sure you want to continue?<br>This action cannot be undone.",
        rejectClose: false,
        modal: true,
      });
      if (!proceed) return;
    ...
    html.on("click", ".ap-interact", (ev) => {
      ...
      if (index === this.actor.system.resources.ap.value) {
        this.actor.update({
          "system.resources.ap.value": this.actor.system.resources.ap.value - 1,
        });
      } else {
    ...
    html.on("click", ".fp-interact", (ev) => {
      ...
      if (index === this.actor.system.resources.fp.value) {
        this.actor.update({
          "system.resources.fp.value": this.actor.system.resources.fp.value - 1,
        });
      } else {
    ...
    html.on("click", ".item-memorize", (ev) => {
      ...
      if (item.system.memorized === "true") {
        ...
        if (
          item.system.memSlots + this.actor.system.attributes.memory.spent >
    ...
    html.on("click", ".armor-socket-browse", async (ev) => {
      ...
      const msg = await foundry.applications.handlebars.renderTemplate(
        "systems/celestus/templates/actor/parts/actor-armor-popup.hbs",
        {
  ...
  _appearanceEdit() {
    new foundry.applications.api.DialogV2({
      ...
      submit: async (values) => {
        if (values.pointerTint)
          await this.actor.update({
            "system.pointerTint": values.pointerTint.value,
          });
        if (values.portraitBorder)
          await this.actor.update({
            "system.portraitBorder": values.portraitBorder.checked,
          });
      },
    }).render({ force: true });
  }
    ...
    new foundry.applications.api.DialogV2({
      ...
      submit: async (values) => {
        for (const [language] of Object.entries(CONFIG.CELESTUS.languages)) {
          await this.actor.update({
            [`system.attributes.languages.${language}`]:
              values[language].checked,
          });
        }
      },
    }).render({ force: true });
...
export class CelestusItemSheet extends foundry.appv1.sheets.ItemSheet {
  ...
  async getData() {
    ...
    const potentialSpreads = CONFIG.CELESTUS.itemSocketSpreads.filter(
      (s) =>
    ...
    for (const spread of potentialSpreads) {
      generation.validSpreads[spread.id] =
        `${spread.id} | Req. lvl ${spread.minLvl}`;
    }
    ...
    for (const i in this.document.system.socketTypes) {
      ...
      if (type) {
        ...
        const sockets = CONFIG.CELESTUS.itemSockets.filter(
          (s) =>
    ...
    // runes stuff
    if (
      ["armor", "offhand", "weapon"].includes(this.item.type) &&
      ...
      for (const rune of actorRunes) {
        context.runes[rune.id] =
          `${rune.name}${rune.system.slotted ? " (slotted)" : ""}`;
      }
    }
 ...
 */
export class CelestusActiveEffectSheet extends foundry.applications.sheets
  .ActiveEffectConfig {
  /** @override */
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
 ...
 */
export class CelestusMeasuredTemplateConfig extends foundry.applications.sheets
