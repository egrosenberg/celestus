...
import {
  CelestusMeasuredTemplate,
  CelestusMeasuredTemplateDocument,
  CelestusTemplateLayer,
} from "./measure.mjs";
...
Hooks.on("init", () => {
  ...
  game.system.initiative =
    "1d20+@abilities.wit.total+@attributes.bonuses.initiative.value+(@abilities.wit.total*0.01)";
  ...
  CONFIG.CELESTUS = {
    ...
    damageTypes: {
      ...
      mag_armor: {
        ...
        skill: "tidecaller",
        color: "#86dfdf",
        glyph: "icon-magic-shield",
      },
      t_phys_armor: {
        label: "Temp Physical Armor",
        text: "t_phys_armor",
        style: "healing",
        skill: "duneshaper",
        color: "#ebccad",
    ...
    combatSkills: {
      ...
      retributive: {
        ...
        text: "retributive",
        damage: "physical",
        glyph: "icon-spiked-shoulder-armor",
        type: "weapon",
      },
      flamespeaker: {
        label: "Flamespeaker",
    ...
    abilityMod: {
      // ability modifer percentages per point above or below 10
    ...
    baseDamage: {
      // base damage for spells / abilities
      formula: {
        // die roll formual
        1: "2d6[none] + 0",
        2: "2d6[none] + 2",
    ...
    gearTypes: ["armor", "weapon", "offhand", "feature"],
    socketable: ["armor", "weapon", "offhand"],
  ...
  $(resourceUi).on("click", ".resource-input", function () {
    this.select();
  });
...
Hooks.on("activateTemplateLayer", (layer) => {
  layer.zIndex = 400;
});
Hooks.on("deactivateTemplateLayer", (layer) => {
  layer.zIndex = CelestusTemplateLayer.layerOptions.zIndex;
});
...
Hooks.on("controlToken", (d, c) => {
  renderHotbarOverlay(c);
});
...
Hooks.on("refreshToken", (token) => {
  token.updatePointerVisibility();
});

// token pointer overlay
Hooks.on("refreshToken", (t) => {
  if (!t._original) t.drawPointer();
});
Hooks.on("canvasDraw", () => {
...
function awaitElevationRuler(timeout) {
  ...
  function waitRuler(resolve, reject) {
    if (CONFIG.elevationruler?.SPEED) resolve(true);
    else if (timeout && Date.now() - start >= timeout) reject(undefined);
    else setTimeout(waitRuler.bind(this, resolve, reject), 30);
  }
}
